...
import {
  ...
  ReactIcon,
  TailwindCSSIcon,
} from "@/components/ncdai/hero-01-icons"
...
function MainContent({ className }: { className?: string }) {
  return (
    ...
    >
      <h1 className="mb-4 font-heading text-[2.5rem]/none font-medium tracking-tight text-foreground sm:mb-6 sm:text-5xl md:text-4xl lg:text-5xl xl:text-6xl">
        Build. Ship. Authenticate.
      </h1>
      ...
      <p className="mb-6 text-base leading-normal! text-muted-foreground sm:mb-8 sm:text-xl sm:text-balance md:text-lg lg:text-xl">
        ...
        software development and Authentication services based on your ideas.
      </p>

      <div className="mb-6 grid grid-cols-2 items-center gap-4 sm:mb-8 sm:flex">
        <Button className="border-none px-4 sm:px-8" size="lg" render={<a href="/signup" />} nativeButton={false}>Sign up</Button>

        <Button className="px-4 sm:px-8" variant="outline" size="lg" render={<a href="/login" />} nativeButton={false}>Log In</Button>
      </div>
      ...
      <div className="relative -ml-4 lg:ml-0">
        ...
        <div className="no-scrollbar flex items-center gap-4 overflow-x-auto px-4 lg:px-0">
          ...
          <TechItem icon={<BetterAuthIcon />} title="Better Auth" />
          <TechItem icon={<DrizzleIcon />} title="Drizzle" />
          <TechItem icon={<NeonIcon />} title="Neon" />
          <TechItem icon={<PostgreSQLIcon />} title="PortgreSQL" />
        </div>
      </div>
    </div>
  )
...
function TechItem({ icon, title }: { icon: JSX.Element; title: string }) {
  return (
    <div className="flex items-center space-x-2 text-muted-foreground select-none [&_svg]:shrink-0 [&_svg]:grayscale-100 [&_svg:not([class*='size-'])]:size-6">
      {icon}
